...
/////////////////////
int	 saveNameAndLengthToBinaryFileCompressed(const char* fileName, const SuperMarket* market)
{
	if (!fileName || !market || !market->name)    //// is it nececcery ?
		return 0;
	FILE* file = fopen(fileName, "wb");

	CHECK_MSG_RETURN_0(file, "Error opening supermarket file to write");

	if (!file)
		return 0;


	int len = strlen(market->name);
	BYTE data[2] = { 0 };
	data[0] = (market->productCount >> 2) & 0xFF;
	data[1] = (market->productCount & 0X3) << 6 | len & 0X3F;

	if (fwrite(&data, sizeof(BYTE), 2, file) != 2)
		return 0;

	if (fwrite(market->name, sizeof(char), len, file) != len)
		return 0;

	for (int i = 0; i < market->productCount; i++)
	{
		if (!saveProductToCompressedFile(file, market->productArr[i]))
		{
			fclose(file);
			return 0;
		}
...
int loadSuperMarketFromFileCompressed(SuperMarket* market, const char* fileName, const char* customersFileName)
{
	if (!fileName || !market)
		return 0;
	...
	CHECK_MSG_RETURN_0(file, "Error opening supermarket file to read");
	...
	if (fread(&dataR, sizeof(BYTE), 2, file) != 2)
		return 0;

	market->productCount = ((dataR[0] << 2) | (dataR[1] >> 6));
	storeNameLen = (dataR[1] & 0x3F);
	market->name = readStringFromFileWithLen(file, storeNameLen, "Error reading supermarket name\n");
	market->productArr = (Product**)malloc(market->productCount * sizeof(Product*));
	if (!market->productArr)
	{
		return 0;
	}
	...
	for (int i = 0; i < market->productCount; i++)
	{
		market->productArr[i] = (Product*)malloc(sizeof(Product));
		if (!market->productArr[i])
		{
			for (int j = 0; j < i; j++)
				free(market->productArr[j]);
			free(market->productArr);
			free(market->name);
			fclose(file);
			return 0;
		}
		if (!loadProductFromCompressedFile(file, market->productArr[i]))
		{
			for (int j = 0; j < i; j++)
				free(market->productArr[j]);
			free(market->productArr);
	...
	market->customerArr = loadCustomersFromTextFile(customersFileName, &market->customerCount);
